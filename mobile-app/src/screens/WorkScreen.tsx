...
export default function WorkScreen() {
  ...
  const [winningDesigns, setWinningDesigns] = useState<Contest[]>([]);
  ...
  const loadWinningDesigns = async () => {
    setIsLoading(true);
    try {
      // Load completed contests with winning designs
      ...
      if (response.ok) {
        ...
        // Filter for completed contests with accepted submissions
        const completedContests = contests.filter((c: Contest) => 
          c.status === 'COMPLETED' && c.acceptedSubmissions > 0
        );
        
        setWinningDesigns(completedContests);
      }
    } catch (error) {
  ...
  const renderWinningDesign = (contest: Contest) => (
    <View key={contest.id} style={styles.designCard}>
      <View style={styles.designHeader}>
        <Text style={styles.designTitle}>{contest.title}</Text>
        <View style={styles.designTags}>
          ...
          <View style={[styles.tag, { backgroundColor: '#3B82F6' }]}>
            <Text style={styles.tagText}>{contest.service}</Text>
          </View>
        </View>
      </View>
      
      <View style={styles.designDetails}>
        <Text style={styles.designDetail}>Winners Selected: {contest.acceptedSubmissions}</Text>
        <Text style={styles.designDetail}>Total Submissions: {contest.totalSubmissions}</Text>
        <Text style={styles.designDetail}>Completed: {new Date(contest.createdAt).toLocaleDateString()}</Text>
      </View>
      
      <TouchableOpacity
        style={styles.viewDesignsButton}
        onPress={() => navigation.navigate('ContestDetails', { contestId: contest.id })}
      >
        <Text style={styles.viewDesignsButtonText}>View Winning Designs</Text>
      </TouchableOpacity>
    </View>
  );
