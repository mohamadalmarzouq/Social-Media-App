...
export default function DashboardScreen() {
  ...
  const cancelContest = async (contestId: string) => {
    try {
      const response = await fetch(`${API_BASE_URL}/contests/${contestId}/cancel`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const renderContestCard = (contest: Contest) => (
    <View key={contest.id} style={styles.contestCard}>
      ...
      <View style={styles.contestDetails}>
        <Text style={styles.contestDetail}>Current Round: Round {contest.currentRound}</Text>
        <Text style={styles.contestDetail}>
          Progress: {contest.acceptedSubmissions}/{contest.expectedSubmissions} designs accepted
        </Text>
        <Text style={styles.contestDetail}>Submissions: {contest.totalSubmissions}</Text>
